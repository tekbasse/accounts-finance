...
ad_proc -public acc_fin::pretti_color_chooser_old2 {
    ...
    if { $on_cp_p > -1 } {
        ...
        if { $on_cp_p } {
            ...
            set color_mask_list $color_sig_mask_list
            set c1 [f::max 0 [f::min 255 [expr { int( 127. * $on_a_sig_path_p + $popularity * $k2 + 1. ) } ] ]]
        }
        set c0 [expr { 255 - $c1 } ]
        if { $odd_row_p } {
            incr c1 $row_contrast 
...
ad_proc -public acc_fin::pretti_color_chooser_old {
    on_cp_p
    on_a_sig_path_p
    odd_row_p
    popularity
...
set k2 [expr {  7. / ( $max_act_count_per_track + 0. ) } ]
    ...
    if { $on_cp_p > -1 } {
        ...
        if { $on_cp_p } {
            ...
            set color_mask_list $color_sig_mask_list
            set c(1) [f::max 1 [f::min 15 [expr { int( ( 7. * $on_a_sig_path_p ) + $popularity * $k2 + 1. ) } ]]]
        }
        set c(0) [expr { 15 - $c(1) } ]
        set colorhex ""
        ...
        if { $odd_row_p } {
            incr c(1) -1
            # incr c(0) [f::sum $color_mask_list]
        } 
        ...
        if { [string length $colorhex] != 6 } {
            ns_log Notice "acc_fin::pretti_color_chooser.914: issue colorhex '$colorhex' on_a_sig_path_p ${on_a_sig_path_p} popularity $popularity on_cp_p $on_cp_p c(0) '$c(0)' c(1) '$c(1)' color_mask_list '${color_mask_list}' odd_row_p '${odd_row_p}"
        }
    } else {
        set colorhex "505050"
    }
